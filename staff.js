...
// === LOAD WEEKLY ATTENDANCE (show week icons + term-wide totals) ===
document.getElementById('loadViewAttendanceBtn').addEventListener('click', async () => {
  ...
  try {
    ...
    // helper to map status to colored icon
    const statusIcon = (status) => {
      if (!status) return `<span title="No record">-</span>`;
      if (status === 'Present') return `<span style="color:green;font-weight:bold;" title="Present">✔</span>`;
      if (status === 'Absent') return `<span style="color:red;font-weight:bold;" title="Absent">✖</span>`;
      return `<span title="${status}">${status[0] || '-'}</span>`;
    };
    ...
    data.data.forEach(stu => {
      ...
      (stu.days || []).forEach(d => {
        ...
        if (dayName) dayMap[dayName] = d.status;
      });

      // Build table cells with icons
      const dayCells = weekDays.map(day => {
        const status = dayMap[day];
        return `<td class="text-center">${statusIcon(status)}</td>`;
      }).join('');

      // === 👇 Term-wide totals from backend 👇 ===
      const totalPresent = stu.total_present_term;
      const totalDays = stu.total_days_term;
      const percent = stu.term_percentage; // computed by backend
