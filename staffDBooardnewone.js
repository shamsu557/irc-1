...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // Optional: attach buttons to functions if needed
  document.getElementById("loadAttendanceBtn")?.addEventListener("click", loadAttendance);
  document.getElementById("saveAttendanceBtn")?.addEventListener("click", saveAttendance);
});
...
window.downloadTahfizReport = async (studentId, session, term, sectionId, classId) => {
  window.open(
    `/api/student-report?student_id=${studentId}&session=${session}&term=${term}&type=tahfiz&section_id=${sectionId}&class_id=${classId}`,
    "_blank",
  )
}
