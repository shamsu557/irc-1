...
async function loadAyatRangeForDay() {
  ...
  try {
    ...
    if (data.success && data.data.length === 1) {
      const scheme = data.data[0]
      window.currentScheme = scheme

      const txt = `Week ${scheme.week} – ${scheme.day}: ${scheme.from_surah_ayah} → ${scheme.to_surah_ayah}`
...
async function selectCurrentAyatRange() {
  if (!window.currentScheme) return alert("No ayat range loaded.")
  ...
  window.selectedSchemeId = window.currentScheme.id
  window.selectedWeek = window.currentScheme.week
  window.selectedDay = window.currentScheme.day

  document.getElementById("selectedAyahDisplay").textContent =
    `Week ${week} – ${day}: ${window.currentScheme.from_surah_ayah} → ${window.currentScheme.to_surah_ayah}`
  document.getElementById("selectedAyahInfo").style.display = "block"

  try {
    const resp = await fetch(
      `/api/staff-memorization/${currentStaffId}?` +
      `section_id=${sectionId}&class_id=${classId}&scheme_id=${window.selectedSchemeId}` +
      `&session=${session}&term=${term}&week=${week}&day=${encodeURIComponent(day)}`
    )
    const data = await resp.json()
    ...
    if (data.success && data.data.length) {
      ...
      data.data.forEach(stu => {
        ...
        row.innerHTML = `
          ...
          <td>
            <select class="form-select final-grade" data-enrollment-id="${stu.enrollment_id}">
              ${["","A","B","C","D","E","F"].map(g =>
...
async function saveMemorization() {
  ...
  if (!classVal || !session || !term || !week || !day || !window.selectedSchemeId) {
