...
// =============================================
// ADMIN: Weekly + Cumulative Attendance – FINAL PERFECT VERSION
// =============================================
...
// Clean class name - removes (Islamic)/(Western)
function getCleanClassName(selectId) {
  const select = document.getElementById(selectId);
  if (!select?.selectedOptions[0]) return "Class";
  return select.selectedOptions[0].textContent.replace(/\s*\([^)]*\)/g, "").trim();
}

// Preload Classes & Sessions — NO BRACKETS AT ALL
async function preloadAdminDropdowns() {
  try {
    const classRes = await fetch("/api/admin-classes");
    const classData = await classRes.json();

    // For Weekly
    const weeklySelect = document.getElementById("adminWeeklyClassSelect");
    const cumulativeSelect = document.getElementById("cumulativeClassSelect");

    weeklySelect.innerHTML = '<option value="">Select Class</option>';
    cumulativeSelect.innerHTML = '<option value="">Select Class</option>';

    classData.data.forEach(c => {
      const opt1 = new Option(c.name, c.value);  // ONLY CLEAN NAME
      const opt2 = new Option(c.name, c.value);
      weeklySelect.add(opt1);
      cumulativeSelect.add(opt2);
    });
    ...
    const weeklySession = document.getElementById("adminWeeklySessionSelect");
    const cumulativeSession = document.getElementById("cumulativeSessionSelect");

    weeklySession.innerHTML = '<option value="">Select Session</option>';
    cumulativeSession.innerHTML = '<option value="">Select Session</option>';

    sessionData.data.forEach(s => {
      weeklySession.add(new Option(s.session_year, s.session_year));
      cumulativeSession.add(new Option(s.session_year, s.session_year));
    });

  } catch (err) {
    console.error("Failed to load dropdowns:", err);
    alert("Error loading classes/sessions. Check server.");
  }
}
...
async function loadAdminWeeklyAttendance() {
  ...
  if (!classId || !session || !term || !week) return alert("Please select all fields");
  ...
  try {
    ...
    processed.forEach(s => {
      ...
      row.innerHTML = `
        ...
        <td>${s.student_id}</td>
        <td class="text-center">${s.Saturday || "-"}</td>
        <td class="text-center">${s.Sunday || "-"}</td>
        <td class="text-center">${s.Monday || "-"}</td>
        <td class="text-center">${s.Tuesday || "-"}</td>
        <td class="text-center">${s.Wednesday || "-"}</td>
        <td class="text-center">${s.Thursday || "-"}</td>
        <td class="text-center">${s.Friday || "-"}</td>
    ...
    console.error(err);
    tbody.innerHTML = `<tr><td colspan="11" class="text-center text-danger">Network/Server Error</td></tr>`;
  }
}
...
async function loadCumulativeAttendance() {
  ...
  try {
    ...
    if (!data.success || data.data.length === 0) {
      tbody.innerHTML = `<tr><td colspan="5" class="text-center text-muted py-8">No attendance data up to Week ${upToWeek}</td></tr>`;
      cumulativeAttendanceData = [];
      return;
    }

    cumulativeAttendanceData = data.data;
    tbody.innerHTML = "";
    data.data.forEach(s => {
      const row = document.createElement("tr");
      row.innerHTML = `
    ...
    console.error(err);
    tbody.innerHTML = `<tr><td colspan="5" class="text-center text-danger">Error loading cumulative data</td></tr>`;
  }
}
...
// Load dropdowns
document.addEventListener("DOMContentLoaded", preloadAdminDropdowns);